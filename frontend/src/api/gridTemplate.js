...
export const gridTemplateAPI = {
     ...
     */
    async list(params = {}) {
        const response = await apiClient.get('/grid-templates', { params });
    ...
    async getDetail(templateId) {
        const response = await apiClient.get(`/grid-templates/${templateId}`);
        return response.data;
    },
    ...
    async useTemplate(templateId, data) {
        const response = await apiClient.post(`/grid-templates/${templateId}/use`, data);
        return response.data;
    },
    ...
    async getFeatured(limit = 5) {
        const response = await apiClient.get('/grid-templates', {
