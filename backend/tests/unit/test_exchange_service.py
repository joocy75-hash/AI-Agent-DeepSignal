...
class TestExchangeService:
    ...
    async def test_has_api_key_returns_false_when_not_exists(
        ...
        result = await ExchangeService.has_api_key(async_session, test_user.id)

        assert result is False

    @pytest.mark.asyncio
    async def test_has_api_key_returns_false_for_nonexistent_user(self, async_session):
    ...
    async def test_get_user_exchange_client_raises_when_no_api_key(
        ...
        with pytest.raises(HTTPException) as exc_info:
            await ExchangeService.get_user_exchange_client(async_session, test_user.id)
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="User.exchange is NOT NULL constraint - cannot test None case"
    )
    async def test_get_user_exchange_client_uses_default_exchange(
        ...
        Note: This test is skipped because User.exchange has NOT NULL constraint.
        The default exchange logic would need to be tested differently.
        """
        pass
